...
const AddNewActor = () => {
    ...
    if (file) {
      try {
        ...
        if (type === 'actor') {
          setFormValues((prev) => ({
            ...prev,
            ActorImages: [...prev.ActorImages, imageUrl], // Add the new image URL to the array
          }));
        } else if (type === 'movie') {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${apiurl}/AddNewActor`, formValues, {
      ...
      <Box sx={{ mt: 5 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="image-container" style={{ display: 'flex', flexWrap: 'wrap' }}>
            {formValues.ActorImages.map((el, index) => (
              <div className="image-wrapper" key={index} style={{ position: 'relative', marginRight: '10px' }}>
                ...
                <div
                  ...
                  onClick={() => setFormValues({
                    ...formValues,
                    ActorImages: formValues.ActorImages.filter((_, i) => i !== index),
                  })}
                  style={{ position: 'absolute', top: 0, right: 0, cursor: 'pointer' }}
                >
                  <MdDelete />
          ...
          <Stack direction="row" spacing={2} sx={{ mt: 3 }}>
            <Button type="submit" variant="contained" color="primary">Add Actor</Button>
